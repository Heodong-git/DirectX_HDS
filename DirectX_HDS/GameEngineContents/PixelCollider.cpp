...
bool PixelCollider::FallCheck()
{
	...
	if (g_WhitePixel == ColPixel)
	{
		...
		Player::MainPlayer->GetTransform()->AddLocalPosition({ float4::Up });
		return true;
	}
...
bool PixelCollider::RightPixelCheck()
{
	...
	{
		// 네거티브스케일x 를 사용해서 애니메이션 방향을 변경중이기 때문에 왼쪽 디버그 픽셀을
		CheckPos = Player::MainPlayer->m_DebugRender_Wall_Left->GetTransform()->GetWorldPosition();
	}
	
	// 정방향이면 그대로 우측픽셀로 체크
	else if (true == Player::MainPlayer->GetDir())
	{
		CheckPos = Player::MainPlayer->m_DebugRender_Wall_Right->GetTransform()->GetWorldPosition();
	}
...
float4 PixelCollider::RightWallCheck(float4& _Pos)
{
	ColMapSetting();
	...
	return float4{ -static_cast<float>(LeftCount) , 0.0f };
}
